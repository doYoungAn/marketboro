...
const Vendor: FC<IVendorProps> = ({ vendor }): JSX.Element => {
  ...
  return (
    <>
      <div className="uk-card uk-card-default uk-card-body uk-card-hover uk-card-small uk-animation-slide-bottom-small">
        ...
        <div data-uk-grid className="uk-grid-small uk-padding-small uk-padding-remove-bottom">
          {vendor.businesses.map((business, index) => (
            <span key={index} className="uk-badge uk-margin-right uk-padding-small">{business.name}</span>
          ))}
        </div>
      </div>
    </>
